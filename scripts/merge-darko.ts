...
/**
 * Result from finding the latest DARKO CSV
 */
interface DarkoCsvResult {
  path: string;
  date: string; // YYYY-MM-DD format from filename
}
 ...
 */
async function findLatestDarkoCsv(): Promise<DarkoCsvResult | null> {
  ...
  if (latestFile && latestDate) {
    return { path: latestFile, date: latestDate };
  }
  return null;
}
...
/**
 * Update the metadata.ts file with the new DARKO date
 *
 * This file provides fallback metadata for Vite dev mode.
 */
async function updateMetadataFile(darkoDate: string) {
...
async function main() {
  ...
  const csvResult = await findLatestDarkoCsv();
  if (!csvResult) {
  ...
  const { path: csvPath, date: csvDate } = csvResult;
  ...
  // Full header: #,Player,Team,Pos,DPM,ODPM,DDPM,Box,On/Off DPM,MPG,Pace,
  //              Pts per 100,Ast per 100,FG%,3P%,FT%,$ Value,Salary,Surplus Value
  const columns = [
    "rank",
    "Player",
    "Team",
  ...
  // Pass the DARKO date from the CSV filename to store in metadata
  await setPlayers(updatedPlayers, { darkoUpdated: csvDate });
  await updateFallbackFile(updatedPlayers);
  await updateMetadataFile(csvDate);
